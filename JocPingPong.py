...
background_image = pygame.image.load("C:\Programare/Background.png")
background_image = pygame.transform.scale(background_image, (LATIME, INALTIME))

background_image2 = pygame.image.load("C:\Programare/background2.jpg")
background_image2 = pygame.transform.scale(background_image2, (LATIME, INALTIME))

background_image3 = pygame.image.load("C:\Programare/background3.jpg")
background_image3 = pygame.transform.scale(background_image3, (LATIME, INALTIME))
FPS = 60
...
def desen(win, palete, minge, scor_stanga, scor_dreapta, cul):
    background_color, paddle_color, ball_color, line_color, background_im = get_color(cul)
...
def select_difficulty():
    ...
    while run:
        ...
        message = get_font(17).render("Selectati dificultatea folosind sagetile", 1, ALB)
        WIN.blit(message, (LATIME // 2 - message.get_width() // 2, INALTIME // 8))

        for i, option in enumerate(difficulty_options):
            text_font=get_font(15)
            text = text_font.render(option, 1, "#b68f40" if selected_difficulty == option else ALB)
...
def get_color(cul):
    if cul == "Pachetul 1":
        return (255, 182, 193), (255, 255, 0), (255, 255, 0), (255, 255, 0), background_image
    elif cul == "Pachetul 2":
        return (169, 169, 169), (0, 128, 0), (0, 128, 0), (0, 128, 0), background_image3
    elif cul == "Pachetul 3":
         return ALB, NEGRU, NEGRU, NEGRU, background_image2
    else:
...
def pagina_start():
    ...
    while run:
        clock.tick(FPS)
        WIN.blit(background_image, (0, 0))
        title = get_font(60).render("PING PONG", True, "#b68f40")
        WIN.blit(title, (LATIME // 2 - title.get_width() // 2, INALTIME // 4))

        start_text = get_font(25).render("Press Enter to start", True, "#b68f40")
        exit_text = get_font(25).render("Press Esc to exit", True, "#b68f40")
...
def select_culoare(default_value ="Pachetul 1"):
    ...
    while run:
        ...
        message = get_font(14).render("Selectati un pachet de culori folosind sagetile", True, "#b68f40")
        WIN.blit(message, (LATIME // 2 - message.get_width() // 2, INALTIME // 8))

        for i, option in enumerate(color_options):
            text_font = get_font(15)
            text = text_font.render(option, 1, "#b68f40" if culoare_selectata == option else ALB)
